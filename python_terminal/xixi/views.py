...
class SearchFrame(Frame):
    ...
    def show_table_search(self):
        columns = ("id", "name", "cl_num", "math", "english", "computer", "total")
    ...
    def search_cl_num(self):
        for _ in map(self.tree_view.delete, self.tree_view.get_children('')):
            pass
        if not self.cl_num_cl_num.get():
            self.show_search_data()
            return
        else:
            self.cl_num= self.cl_num_cl_num.get()
            students = mysql_student.search_cl_num(self.cl_num_value)

        index = -1
        for stu in students:
            self.tree_view.insert('', index + 1, values = (
                stu['id'], stu['name'], stu['cl_num'], stu['math'],
...
class DeleteFrame(Frame):
    def __init__(self, root):
        ...
        Label(self, text = '班   级 :', textvariable=self.cl_num).place(x=360, y=80)
    ...
    def id_delete(self):
        if self.delete_student.get():
            ...
            if flag:
                ...
                self.cl_num.set(stu[0][2]),self.math.set(stu[0][3])
    ...
    def name_delete(self):
        if self.delete_student.get():
            self.search_admin = self.delete_student.get()
            flag, stu = mysql_student.search_name(self.search_admin)
            if flag:
                self.id.set(stu[0][0])
                self.name.set(stu[0][1])
                self.cl_num.set(stu[0][2])
...
class ChangeFrame(Frame):
    def __init__(self, root):
        ...
        self.cl_num_change_before = StringVar()
    ...
    def insert_page(self):
        ...
        Label(self, textvariable=self.cl_num_change_before).place(x=300, y=80)
        self.entry_kulas = Entry(self, textvariable=self.cl_num)
    ...
    def name_change(self):
        if self.change_student.get():
            self.search_admin= self.change_student.get()
            flag, stu = mysql_student.search_name(self.search_admin)
            if flag:
                self.change_Information(stu)
                self.status_student.set('数据查询成功')
    ...
    def change_Information(self, stu):
        ...
        self.cl_num_change_before.set(stu[0][2])
    ...
    def create_user(self):
        ...
        if not self.cl_num.get():
            self.insert_kulas = 'NULL'
        else:
            self.insert_cl_num = self.cl_num.get()
